...
const fetchZones = async () => {
  ...
  if (filters.value.company) {
    const res = await api.post('market/zone-by-company', { company: filters.value.company })
...
const fetchWings = async () => {
  ...
  if (filters.value.zone) {
    const res = await api.post('market/wing-by-zone', { zone: filters.value.zone })
...
const fetchDivisions = async () => {
  ...
  if (filters.value.wing) {
    const res = await api.post('market/division-by-wing', { wing: filters.value.wing })
...
const fetchRegions = async () => {
  ...
  if (filters.value.division) {
    const res = await api.post('market/region-by-division', { division: filters.value.division })
...
const fetchThanas = async () => {
  ...
  if (filters.value.territory) {
    const res = await api.post('market/thana-by-territory', { territory: filters.value.territory })
...
const fetchSPsAndDistributors = async () => {
  ...
  if (filters.value.thana) {
    const [spRes, disRes] = await Promise.all([
      api.post('market/sp_name-by-thana', { thana: filters.value.thana }),
      api.post('market/distributor-by-thana', { thana: filters.value.thana }),
    ])
