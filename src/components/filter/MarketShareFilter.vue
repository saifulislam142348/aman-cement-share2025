<template>
  <div class="pb-4 pt-4 overflow-x-auto">
    <div class="flex flex-nowrap gap-1 min-w-max">
      ...
      <!-- Division -->
      <el-select v-model="localFilters.division" placeholder="Select Division" clearable filterable remote reserve-keyword
        :remote-method="searchDivisions" :loading="loading.division" class="w-48" @change="fetchRegions">
        <el-option v-for="item in divisions" :key="item" :label="item" :value="item" />
      </el-select>

      <!-- Region -->
      <el-select v-model="localFilters.region" placeholder="Select Region" clearable filterable remote reserve-keyword
        :remote-method="searchRegions" :loading="loading.region" class="w-48" @change="fetchAreas">
      ...
      <!-- Territory -->
      <el-select v-model="localFilters.territory" placeholder="Select Territory" clearable filterable remote reserve-keyword
        :remote-method="searchTerritories" :loading="loading.territory" class="w-48" @change="fetchThanas">
        <el-option v-for="item in territories" :key="item" :label="item" :value="item" />
      </el-select>

      <!-- Thana -->
      <el-select v-model="localFilters.thana" placeholder="Select Thana" clearable filterable remote reserve-keyword
        :remote-method="searchThanas" :loading="loading.thana" class="w-48" @change="fetchRetailers">
        <el-option v-for="item in thanas" :key="item" :label="item" :value="item" />
      </el-select>

      <!-- Retailer -->
      <el-select v-model="localFilters.retailer" placeholder="Select Retailer" clearable filterable remote reserve-keyword
        :remote-method="searchRetailers" :loading="loading.retailer" class="w-48" @change="emitFilter">
