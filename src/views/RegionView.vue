      ...
      for (const wing of zone.wings) {
        ...
        for (const division of wing.divisions) {
          ...
          for (const region of division.regions) {
            ...
            if (!Array.isArray(region.months)) continue

            for (const month of region.months) {
              rows.push({
                id: `${companyName}-${zoneName}-${wingName}-${divisionName}-${regionName}-${month.month}-${month.year}`,
                company: companyName,
                zone: zoneName,
                wing: wingName,
...
// Calculate subtotals per month-year group
...
async function fetchData() {
  try {
    // axios automatically converts { params } into query string
    const { data } = await api.get('market/region', {
    ...
    console.error('Region fetch error:', e.response?.data || e.message || e)
  }
}
watchEffect(() => {
  if (filters.value) {
